      ...
      <main className="max-w-lg mx-auto px-4 py-10">
        ...
        <div className="mb-10 text-center">
          ...
          <h1 className="text-2xl font-bold text-foreground mb-3">
            {t('install.app_coming_soon', 'App bald verfügbar')}
          </h1>
          <p className="text-muted-foreground leading-relaxed text-sm">
            {t('install.store_launch_desc', 'Neighbors Kitchen erscheint bald im App Store und bei Google Play.')}
          </p>
          <p className="text-muted-foreground leading-relaxed text-sm mt-1">
            {t('install.notify_when_ready', 'Wir geben dir Bescheid, sobald es so weit ist.')}
          </p>
        </div>
        ...
        <div className="space-y-3 mb-4">
          ...
          <div className="w-full h-14 rounded-xl bg-black flex items-center justify-center gap-3 opacity-50 cursor-default pointer-events-none px-6">
            ...
            <div className="text-left text-white">
              <div className="text-[10px] leading-tight">{t('install.download_from', 'Laden im')}</div>
          ...
          <div className="w-full h-14 rounded-xl bg-black flex items-center justify-center gap-3 opacity-50 cursor-default pointer-events-none px-6">
            ...
            <div className="text-left text-white">
              <div className="text-[10px] leading-tight">{t('install.get_it_on', 'Jetzt bei')}</div>
        ...
        <p className="text-xs text-muted-foreground/60 text-center mb-10">
          {t('install.official_release', 'Offizielle App-Store-Version. Kein Beta-Zugang.')}
        </p>
          ...
          <CollapsibleContent className="space-y-4 mt-2">
            <Card className="border-muted">
              <CardContent className="pt-4 space-y-2 text-xs text-muted-foreground">
                ...
                <p>1. {t('install.ios_step1', 'Tippe auf')} <Share2 className="inline w-3 h-3" /> {t('install.ios_share_menu', '(Teilen-Menü)')}</p>
                <p>2. {t('install.ios_step2', 'Wähle "Zum Home-Bildschirm"')}</p>
                <p>3. {t('install.ios_step3', 'Bestätigen')}</p>
              </CardContent>
            </Card>
            <Card className="border-muted">
              <CardContent className="pt-4 space-y-2 text-xs text-muted-foreground">
                <p className="font-medium text-foreground text-sm">🤖 Android (Chrome)</p>
                <p>1. {t('install.android_step1', 'Tippe auf')} <Menu className="inline w-3 h-3" /> {t('install.android_menu', '(Menü)')}</p>
                <p>2. {t('install.android_step2', 'Wähle "App installieren"')}</p>
                <p>3. {t('install.android_step3', 'Bestätigen')}</p>
              </CardContent>
            </Card>
          </CollapsibleContent>
        </Collapsible>
        ...
        <div className="mt-10 text-center">
          <Button onClick={() => navigate('/feed')} size="lg" className="w-full">
            {t('install.continue_to_app', 'Weiter zu Neighbors Kitchen')}
          </Button>
        </div>
      </main>
    </div>
