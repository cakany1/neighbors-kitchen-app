...
const AppRoutes = () => {
  ...
  const [showEnvMismatchError, setShowEnvMismatchError] = useState(false);
  const location = useLocation();

  // Initialize analytics tracking
  useAnalyticsTracking();

  // SAFETY GUARD: Verify environment is correctly isolated (runs once on mount)
  useEffect(() => {
    ...
    setShowEnvMismatchError(mismatch);
  }, []);
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="max-w-md bg-card rounded-lg border-2 border-destructive p-6 space-y-4">
          ...
          <p className="text-destructive/90 font-mono text-sm">
            This instance is connected to an unexpected Supabase project.
          </p>

          <p className="text-destructive/90 text-sm">This is a critical safety violation. All requests are BLOCKED.</p>
          ...
          <p className="text-xs text-muted-foreground">
            Contact your team lead. Staging must use a separate Supabase project, and production must only use the
            production project.
          </p>
        </div>
      </div>
